...
from pykeen.triples.leakage import (
    ...
    _translate_triples,
    get_candidate_pairs,
    jaccard_similarity_scipy,
    mapped_triples_to_sparse_matrices,
...
class TestLeakage(unittest.TestCase):
    ...
    def test_generate_compact_vectorized_lookup(self):
        ...
        label_to_id = {f"e_{i}": i for i in range(2 * max_id)}
    ...
    def test_translate_triples(self):
        ...
        num_triples = 31
        triples = torch.stack(
            [2 * torch.randint(max_id, size=(num_triples,)) for max_id in (max_e_id, max_r_id, max_e_id)], dim=-1
        )
