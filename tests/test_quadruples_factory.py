...
from pykeen.temporal.datasets import SmallSample
from pykeen.quadruples import LCWAQuadrupleInstances, QuadruplesFactory
from pykeen.quadruples.quadruples_factory import INVERSE_SUFFIX, QUADRUPLES_DF_COLUMNS
...
class TestQuadruplesFactory(unittest.TestCase):
    ...
    def test_automatic_incomplete_inverse_detection(self):
        ...
        for create_inverse_quadruples in (False, True):
            with patch("pykeen_temporal.quadruples.quadruples_factory.logger.warning") as warning:
                factory = QuadruplesFactory.from_labeled_quadruples(
                ...
                assert factory.num_quadruples == 2
                # check for correct inverse quadruples flag
                assert factory.create_inverse_quadruples == create_inverse_quadruples
    ...
    def _test_restriction(
        ...
        assert (
            original_quadruples_factory.create_inverse_quadruples
            == restricted_quadruples_factory.create_inverse_quadruples
        )

        # verify that the label-to-ID mapping has not been changed
        assert (
            original_quadruples_factory.entity_to_id == restricted_quadruples_factory.entity_to_id
        )
        assert (
    ...
    def test_new_with_restriction(self):
        ...
        timestamp_restrictions = {"2000-01-01", "2000-01-02"}
