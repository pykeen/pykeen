...
We can map a triples factory's entities to identifiers using :func:`~pykeen.triples.TriplesFactory.entities_to_ids` like in the
following example:

.. literalinclude:: ../examples/first_steps/entity_and_relation_mapping.py
    :lines: 4-11,38-39

Similarly, we can map a triples factory's relations to identifiers using :meth:`~pykeen.triples.TriplesFactory.relations_to_ids` like in
