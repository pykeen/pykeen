...
:class:`~pykeen.triples.TriplesFactory` for the training triples, and then passing its entity and relation to the ID
mapping to create a :class:`~pykeen.triples.TriplesFactory` for the evaluation triples. This will filter out any triple
with an unknown ID (and warn you about it, so you know).
