...
position was corrupted, and the training loop scores them via :meth:`~pykeen.models.Model.score_hrt`, one independent
triple at a time.

However, negatives generated by corrupting the same position share the other two components. This means we can group the
negatives by corrupted position and score each group with a single call to :meth:`~pykeen.models.Model.score_h`,
:meth:`~pykeen.models.Model.score_r`, or :meth:`~pykeen.models.Model.score_t` (cf. :ref:`tuple_broadcasting`), instead
of scoring `num_negs_per_pos` independent triples via :meth:`~pykeen.models.Model.score_hrt`. This avoids redundant
representation lookups and, for models with expensive interaction functions or encoders (e.g.,
