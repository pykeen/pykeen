...
Grouped sLCWA Training
----------------------

During sLCWA training, each positive triple $(h, r, t)$ is corrupted by replacing exactly one of $h$, $r$, or $t$ to
obtain a negative triple. By default, the negative sampler returns these negatives as dense triples, discarding *which*
position was corrupted, and the training loop scores them via :func:`pykeen.models.base.Model.score_hrt`, one
independent triple at a time.

However, negatives generated by corrupting the same position share the other two components. This means we can group the
negatives by corrupted position and score each group with a single call to :func:`pykeen.models.base.Model.score_h`,
:func:`~pykeen.models.base.Model.score_r`, or :func:`~pykeen.models.base.Model.score_t` (cf. :ref:`tuple_broadcasting`),
instead of scoring `num_negs_per_pos` independent triples via :func:`~pykeen.models.base.Model.score_hrt`. This avoids
redundant representation lookups and, for models with expensive interaction functions or encoders (e.g.,
:class:`~pykeen.models.ConvE`, NodePiece, message passing models), redundant interaction preprocessing.

This is enabled by passing ``grouped=True`` to :class:`~pykeen.training.SLCWATrainingLoop`, e.g.,
...
.. warning::
    ...
    Grouped sLCWA training is opt-in (``grouped=False`` by default) for two reasons:

    1. The per-target split of negatives differs between the grouped and the dense path (cf.
       :class:`~pykeen.sampling.BasicNegativeSampler`): grouping splits ``num_negs_per_pos`` *per positive*, so every
       positive gets the same target mix, whereas the dense path splits the flattened ``(b·k)`` axis into contiguous
       chunks, so different positives within a batch may get a different target mix. As a result, results obtained with
       ``grouped=True`` are not bit-identical to ``grouped=False`` runs with the same random seed.
    2. Models with batch normalization (e.g. :class:`~pykeen.models.ConvE`) see different batch statistics under grouped
       training, since the encoder then runs over `b` items instead of `b · num_negs_per_pos` items - the same situation
       LCWA training is already in.
