...
graph embedding models. PyKEEN does _not_ implement a training loop with the CWA.
...
This leads to over-generalization and is therefore usually a poor choice for training knowledge
graph embedding models [nickel2016review]_. PyKEEN does _not_ implement a training loop
with the OWA.
...
-----------------------------
The local closed world assumption (LCWA) was introduced by [dong2014]_ and used in
subsequent works as an approach to generate negative examples during training~\cite{Dettmers2018,balavzevic2019tucker}.
...
Two common approaches for generating negative samples are \ac{uns}~\cite{Bordes2013} and \ac{bns}~\cite{Wang2014} in which negative triples are created by corrupting a positive triple \begin{math}(h,r,t) \in \mathcal{K}\end{math} by replacing either \begin{math}h\end{math} or \begin{math}t\end{math}.
We denote with $\mathcal{N}$ %(defined in Equation~\eqref{eq:negative_triples})
the set of all potential negative triples:

\begin{eqnarray}\label{eq:negative_triples}
...
Theoretically, we would need to exclude all positive triples from this set of candidates for negative triples, i.e., $\mathcal{N}^- = \mathcal{N} \setminus \mathcal{K}$.
In practice, however, since usually $|\mathcal{N}| \gg |\mathcal{K}|$, the likelihood of generating a false negative is rather low.
Therefore, the additional filter step is often omitted to lower computational cost.
It should be taken into account that a corrupted triple that is \textit{not part }of the \ac{kg} can represent a true fact.
