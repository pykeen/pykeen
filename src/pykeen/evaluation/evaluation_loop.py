...
def cat_sparse(tensors: List[torch.Tensor]) -> torch.Tensor:
    batch_indices = torch.cat(
        [torch.full(size=(len(tensor),), fill_value=i, dtype=torch.long) for i, tensor in enumerate(tensors)]
    )
    tensors = torch.cat(tensors)
    return torch.stack([batch_indices, tensors], dim=-1)


class TargetedLCWAEvaluationDataset(torch.utils.data.Dataset):
    """A dataset for evaluation of link prediction with 1:n scoring for a given prediction target."""
    ...
    def __init__(
        ...
        # filter positives
        self.filter_offsets = filter_offsets
    ...
    @classmethod
    def from_df(
        cls, df: pandas.DataFrame, df_filter: pandas.DataFrame, target: Target
    ) -> "TargetedLCWAEvaluationDataset":
        key_columns = [c for c in df.columns if c != target]
        unique_eval_keys = df[key_columns].sort_values(by=key_columns).drop_duplicates()
        num_keys = len(unique_eval_keys)
        # filter filter-triples dataframe
        df_filter = pandas.merge(df_filter, unique_eval_keys, on=key_columns, how="inner")
        df = df.copy()
        df["kind"] = "target"
        df_filter["kind"] = "filter"
        df = pandas.concat([df, df_filter], ignore_index=True)
        keys = torch.empty(num_keys, 2, dtype=torch.long)
        kinds = ("target", "filter")
        csr = {
            kind: {
                "offsets": torch.zeros(num_keys + 1, dtype=torch.long),
                "values": [],
                "offset": 0,
            }
            for kind in kinds
        }
        for i, (key, group) in enumerate(df.groupby(by=key_columns)):
            # key
            keys[i] = torch.as_tensor(key)
            for kind in kinds:
                this_csr = csr[kind]
                ids = torch.as_tensor(group.loc[group["kind"] == kind, target].values)
                this_csr["values"].append(ids)
                this_csr["offset"] += len(ids)
                this_csr["offsets"][i + 1] = this_csr["offset"]
        return LCWAEvaluationDataset(
            keys=keys,
            target_offsets=csr["target"]["offsets"],
            target_values=torch.cat(csr["target"]["values"]),
            filter_offsets=csr["filter"]["offsets"],
            filter_values=torch.cat(csr["filter"]["values"]),
            target=target,
        )
    ...
    def prepare_for_targets(
        ...
        additional_filter_triples: Optional[OneOrSequence[MappedTriples]] = None,
        targets: Collection[Target] = (LABEL_HEAD, LABEL_TAIL),
    ) -> Mapping[Target, "TargetedLCWAEvaluationDataset"]:
        if additional_filter_triples is None:
            additional_filter_triples = []
        additional_filter_triples = upgrade_to_sequence(additional_filter_triples)
        if additional_filter_triples:
            additional_filter_triples = torch.cat(additional_filter_triples).unique(dim=0).numpy()
        columns = [LABEL_HEAD, LABEL_RELATION, LABEL_TAIL]
        df = pandas.DataFrame(data=evaluation_triples.numpy(), columns=columns)
        df_filter = pandas.DataFrame(data=additional_filter_triples, columns=columns)
        result = {}
        for target in targets:
            result[target] = TargetedLCWAEvaluationDataset.from_df(df=df, df_filter=df_filter, target=target)
        return result
    ...
    def __getitem__(self, item: int) -> LCWABatch:
        ...
        key = self.keys[item]
        # get targets
        start, stop = self.target_offsets[item : item + 2]
        targets = self.target_values[start:stop]
        # get filter indices
        start, stop = self.filter_offsets[item : item + 2]
        filter_indices = self.filter_values[start:stop]
        return LCWABatch(key=key, targets=targets, filter_indices=filter_indices)

    def __len__(self) -> int:
        return len(self.keys)

    @staticmethod
    def collate(batch_elements: List[LCWABatch]) -> LCWABatch:
        # key, targets
        keys, targets, filter_indices = list(zip(*batch_elements))
        keys = torch.stack(keys)
        targets = cat_sparse(targets)
        filter_indices = cat_sparse(filter_indices)
        return LCWABatch(key=keys, targets=targets, filter_indices=filter_indices)

    def iter_extra_repr(self) -> Iterable[str]:
        yield f"num_keys={len(self.keys)}"
        yield f"num_targets={len(self.target_values)}"
        yield f"num_filters={len(self.filter_values)}"

    def __repr__(self) -> str:
        return f"{self.__class__.__name__}({', '.join(self.iter_extra_repr())})"
