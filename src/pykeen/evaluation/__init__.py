...
However, given the fact that both the training and the test set usually only contain true facts, negative samples are
generated by corrupting these positive triples. For each test triple $t^{+} = (h,r,t) \in \mathcal{K}_{test}$ two sets
of corrupted triples are constructed:
