...
class VisionDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        transforms.append(torchvision.transforms.ConvertImageDtype(torch.get_default_dtype()))
...
class VisualRepresentation(Representation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(encoder, str):
            cls = getattr(torchvision.models, encoder)
            encoder = cls(encoder_kwargs or {})

        pool = functools.partial(torch.mean, dim=(-1, -2))

        encoder = torchvision.models.feature_extraction.create_feature_extractor(
    ...
    @staticmethod
    def _encode(
        images: torch.FloatTensor, encoder: nn.Module, pool: Callable[[torch.FloatTensor], torch.FloatTensor]
    ) -> torch.FloatTensor:
        """
