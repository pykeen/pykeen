...
def kullback_leibler_similarity(
    ...
    result = (e_var / r_var_safe).sum(dim=-1)
    ...
    # = mu**2 / sigma_1
    mu = r_mean - e_mean
    result = result + (mu.pow(2) / r_var_safe).sum(dim=-1)

    # 3. Component
    if exact:
        result = result - e_shape[-1]
    ...
    result = result + r_var_safe.log().sum(dim=-1) - e_var_safe.log().sum(dim=-1)
