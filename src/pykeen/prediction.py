...
def predict_tails_labeled(model: Model, head: str, relation: str):
    """Predict and prioritize possible tails for the given labeled head and relation."""
    head_id = model.triples_factory.entity_to_id[head]
    relation_id = model.triples_factory.relation_to_id[relation]
    return predict_tails(model, head_id, relation_id)


def predict_tails(model: Model, head, relation):
    """Predict and prioritize possible tails for the given head and relation indexes."""
    triples = np.array([
        (head, relation, tail_id)
        for tail_id in model.triples_factory.entity_to_id.values()
    ])
...
def make_predictions(
    ...
    mapped_triples, _, _ = create_mapped_triples(all_triples, entity_label_to_id=entity_to_id,
