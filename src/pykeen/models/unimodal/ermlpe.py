...
class ERMLPE(EntityRelationEmbeddingModel):
    ...
    def score_hrt(self, hrt_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        # Get embeddings
        h = self.entity_embeddings(index=hrt_batch[:, 0]).view(-1, self.embedding_dim)
        r = self.relation_embeddings(index=hrt_batch[:, 1]).view(-1, self.embedding_dim)
    ...
    def score_t(self, hr_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        h = self.entity_embeddings(index=hr_batch[:, 0]).view(-1, self.embedding_dim)
        r = self.relation_embeddings(index=hr_batch[:, 1]).view(-1, self.embedding_dim)
        t = self.entity_embeddings(index=None).transpose(1, 0)
    ...
    def score_h(self, rt_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        ...
        r = self.relation_embeddings(index=rt_batch[:, 0]).view(-1, self.embedding_dim)
        t = self.entity_embeddings(index=rt_batch[:, 1]).view(-1, self.embedding_dim)
