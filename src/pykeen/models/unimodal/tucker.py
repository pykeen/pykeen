...
class TuckER(EntityRelationEmbeddingModel):
    ...
    def score_hrt(self, hrt_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        # Get embeddings
        h = self.entity_embeddings(index=hrt_batch[:, 0]).unsqueeze(1)
        r = self.relation_embeddings(index=hrt_batch[:, 1])
        t = self.entity_embeddings(index=hrt_batch[:, 2]).unsqueeze(1)
    ...
    def score_t(self, hr_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        # Get embeddings
        h = self.entity_embeddings(index=hr_batch[:, 0]).unsqueeze(1)
        r = self.relation_embeddings(index=hr_batch[:, 1])
        t = self.entity_embeddings(index=None).unsqueeze(0)
    ...
    def score_h(self, rt_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        # Get embeddings
        h = self.entity_embeddings(index=None).unsqueeze(0)
        r = self.relation_embeddings(index=rt_batch[:, 0])
        t = self.entity_embeddings(index=rt_batch[:, 1]).unsqueeze(1)
