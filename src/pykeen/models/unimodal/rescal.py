...
class RESCAL(EntityRelationEmbeddingModel):
    ...
    def score_hrt(self, hrt_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        # Get embeddings
        # shape: (b, d)
        h = self.entity_embeddings(index=hrt_batch[:, 0]).view(-1, 1, self.embedding_dim)
        # shape: (b, d, d)
        r = self.relation_embeddings(index=hrt_batch[:, 1]).view(-1, self.embedding_dim, self.embedding_dim)
        # shape: (b, d)
    ...
    def score_t(self, hr_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        ...
        r = self.relation_embeddings(index=hr_batch[:, 1]).view(-1, self.embedding_dim, self.embedding_dim)
        t = self.entity_embeddings(index=None).transpose(0, 1).view(1, self.embedding_dim, self.num_entities)
    ...
    def score_h(self, rt_batch: torch.LongTensor) -> torch.FloatTensor:  # noqa: D102
        ...
        # Get embeddings
        h = self.entity_embeddings(index=None).view(1, self.num_entities, self.embedding_dim)
        r = self.relation_embeddings(index=rt_batch[:, 0]).view(-1, self.embedding_dim, self.embedding_dim)
