...
class ConvKB(EntityRelationEmbeddingModel):
    ...
    ConvKB uses a \ac{cnn} whose feature maps capture global interactions of the input. Each triple
    $(h,r,t) \in \mathbb{K}$ is represented as a input matrix
    $\textbf{A} = [\textbf{h}; \textbf{r}; \textbf{t}] \in \mathbb{R}^{d \times 3}$ in which the columns represent
    ...
    $\textbf{A}$ creating a feature map $\textbf{v}_i = [v_{i,1},...,v_{i,d}] \in \mathbb{R}^d$:

    .. math::

        \textbf{v}_i = g(\omega_j \textbf{A} + \textbf{b})

    where $\textbf{b} \in \mathbb{R}$ denotes a bias term and $g$ an activation function which is employed element-wise.
    Based on the resulting feature maps $\textbf{v}_1, \dots, \textbf{v}_{\tau}$, the plausibility score of a triple
    is given by:

    .. math::

        f(h,r,t) = [\textbf{v}_i; \ldots ;\textbf{v}_\tau] \cdot \textbf{w}

    where $[\textbf{v}_i; \ldots ;\textbf{v}_\tau] \in \mathbb{R}^{\tau d \times 1}$ and
    $\textbf{w} \in \mathbb{R}^{\tau d \times 1} $ is a shared weight vector.
