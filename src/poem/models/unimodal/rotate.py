...
class RotatE(BaseOWAModule):
    """An implementation of RotatE [Sun2019].

     This model uses models relations as cotations in complex plane.

    .. [nickel2016] RotatE: Knowledge Graph Embeddings by relational rotation in complex space
                    Z. Sun and  Z.H. Dong and J.Y. Nie and J. Tang
                    <https://arxiv.org/pdf/1902.10197v1.pdf>
                    ICLR 2019.
    ...
    def __init__(self, num_entities, num_relations, embedding_dim=200,
        ...
        super(RotatE, self).__init__(num_entities, num_relations, criterion, 2*embedding_dim, preferred_device)
    ...
    def apply_forward_constraints(self):
        ...
        with torch.no_grad():
            ...
            norms = torch.norm(self.relation_embeddings.weight.view(self.num_relations, self.embedding_dim, 2), dim=-1, p=2, keepdim=True).view(self.num_relations, 2*self.embedding_dim)
